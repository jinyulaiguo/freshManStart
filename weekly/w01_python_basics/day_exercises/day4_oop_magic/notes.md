...
* **双下划线前缀 `__name`**（无双下划线后缀）：代表“私有成员（private）”。Python 会启动 **名称修饰（Name Mangling）** 机制，在底层将该变量名重命名为 `_ClassName__name`。
  > ⚠️ **避坑警示（私有属性 vs 魔法属性）**：
  > 很多开发者会混淆**仅前缀双下划线**的私有成员与**前后都有双下划线**的特殊成员（通常称为 Dunder）：
  > 1. **私有属性（仅前缀 `__name`）**：无后缀双下划线，会被自动重命名（Name Mangling），外部直接访问会报错 `AttributeError`。
  > 2. **魔法属性/方法（前后包围 `__name__`）**：前后都有双下划线（如 `__class__`、`__name__`、`__init__`），**永远不会**被重命名，是完全公开的系统预留属性，外部可以直接读取（如通过 `obj.__class__.__name__` 获得类名）。
  > 3. **最佳实践**：我们自己编写代码时，**绝对不要**自定义前后都有双下划线的属性（如 `self.__my_val__ = 1`），因为这会破坏命名规范，且无法起到私有保护的效果。
