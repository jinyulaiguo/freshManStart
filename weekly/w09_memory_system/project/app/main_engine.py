...
class MemoryAgentEngine:
    ...
    async def handle_message(self, session_id: str, user_id: str, query: str) -> Dict[str, Any]:
        ...
        # 2. 状态热重构判定（若 session_id 变动，或内存活跃消息及摘要为空，执行断电状态载入）
        if self.buffer_manager.current_session_id != session_id or (not self.buffer_manager.messages and not self.buffer_manager.current_summary):
            self.log_audit("info", f"检测到 Session '{session_id}' 状态变更或未载入，执行状态载入重构...")
