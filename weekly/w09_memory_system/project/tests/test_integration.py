...
async def test_integration_and_isolation():
    ...
    try:
        ...
        # 过滤 <think>...</think> 思考链内容，仅对最终呈现给用户的回答进行安全审查
        clean_reply = re.sub(r"<think>.*?</think>", "", res_b2['reply'], flags=re.DOTALL).strip()
        assert "Go" in clean_reply or "go" in clean_reply.lower(), "张三的回复应当指明是 Go 语言"
        assert "李明" not in clean_reply, "张三的回复绝对不能泄露李明的数据！"
